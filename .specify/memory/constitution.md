# Universe Kids Race Constitution

## Core Principles

### I. 子供ファースト
対象年齢5〜10歳。すべてのUI・ゲームデザインは子供が直感的に
理解できることを最優先する。テキストは最小限、ひらがな・絵文字・
アニメーションで伝える。失敗にネガティブな表現は使わない。

### II. デザイン
「のりものリズムパレード」と統一したビジュアルスタイルを基調とする。
フォントは Zen Maru Gothic、UIは大きな角丸カードと柔らかいシャドウ。
宇宙テーマに合わせ濃い青ベースの配色とし、カラフルな星・惑星で
彩る。3Dモデルはカートゥーン調とする。

### III. シンプル操作
iPad タッチ操作に最適化する。操作は左右移動のみ。
複雑なコンボ・同時押し・長押しは一切なし。
ゲームプレイ・リザルト・カスタマイズを含む全画面UIは、画面スクロールなしで完結できるように設計する。
全画面UIの実用枠は iPad landscape 1024×768 CSS px を基準に、左右 safe-area 24px と上下のブラウザUI/余白 48px を差し引いた 1000×720px 以内とする。主要操作ボタンは下部84px以内の1列アクション行に収め、縦スクロールやネストしたスクロール領域を前提にしない。

### IV. 3D 冒険体験
Three.js を用いた3D描画で、宇宙の奥に向かって進む没入感を
重視する。iPad Safari で 60fps を維持する。

### V. 技術スタック
- Three.js + TypeScript + Vite
- iPad Safari を唯一のターゲットとする
- 外部ライブラリは Three.js 以外は原則追加しない
- YAGNI 原則を徹底する

### VI. 開発ワークフロー
TDD で開発する。各ゲーム機能は独立したモジュールとして作る。
コードはシンプルで読みやすく保つ。

## Governance
この Constitution はプロジェクトの最上位ルールとする。
原則の変更は変更理由と影響範囲を明記した上で行う。

- Amendment 1.1.0 (2026-05-05): iPadでの没入感を保つため、全画面UIをスクロール不要で完結させる原則を追加。影響範囲はゲーム内オーバーレイ、リザルト、カスタマイズ画面。
- Amendment 1.2.0 (2026-05-05): スクロール不要UIの具体枠を 1000×720px として再計算し、主要操作を下部84px以内の1列アクション行に収める原則を追加。影響範囲はリザルト、図鑑、カスタマイズ、タイトル画面。

**Version**: 1.2.0 | **Ratified**: 2026-03-15 | **Last Amended**: 2026-05-05
